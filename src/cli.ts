...
function sortTickets(tickets: Set<string>, pattern: RegExp): string[] {
...
export function formatReport(opts: ReportOptions): string {
  ...
  if (result.warnings.length > 0) {
    lines.push(
      `  ⚠️  Cross-check WARNING: ${result.warnings.length} mismatch(es)`,
    );
  ...
  lines.push(`── Commits: ${ours} (${result.onlyInOurs.length}) vs ${theirs} (${result.onlyInTheirs.length}) ──`);
  lines.push("");
  ...
  if (ticketPattern && ticketUrl) {
    ...
    const oursSorted = sortTickets(oursInfo.tickets, ticketPattern);
    const theirsSorted = sortTickets(theirsInfo.tickets, ticketPattern);
    const allNoTicket = [...oursInfo.noTicket, ...theirsInfo.noTicket];

    lines.push(`── Tickets: ${ours} (${oursSorted.length}) vs ${theirs} (${theirsSorted.length}) ──`);
    lines.push("");
...
function main() {
  ...
  console.log(
    formatReport({
      ...
      ticketPattern: ticketPatternStr
        ? new RegExp(ticketPatternStr)
        : undefined,
      ticketUrl,
    }),
  );
